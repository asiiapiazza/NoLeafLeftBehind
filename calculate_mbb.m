function [bbox, areaMBB] = calculate_mbb(obj_mask)
[y, x] = find(obj_mask);
points = [x, y];

% Trova il Convex Hull
k = convhull(points);

% Prende solo i punti del Convex Hull
hullPoints = points(k, :);

% Trova la Bounding Box minima sui punti del Convex Hull
bbox = minBoundingBox(hullPoints');

% Trova la lunghezza dei lati della bounding box
side1 = norm(bbox(:,1) - bbox(:,2));
side2 = norm(bbox(:,2) - bbox(:,3));
areaMBB = side1 * side2;
...
% Link: https://it.mathworks.com/matlabcentral/fileexchange/31126-2d-minimal-bounding-box
